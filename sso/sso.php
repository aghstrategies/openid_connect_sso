...
// Load configuration.
if (!file_exists(__DIR__ . '/sso.config.php')) {
  die('Please create and configure sso.config.php');
}
else {
  include __DIR__ . '/sso.config.php';
  if (!isset($network) | !is_array($network)) {
    die('Please configure the network of sites in sso.config.php');
  }
}
...
function sso_debug($message = NULL) {
  syslog(LOG_DEBUG, sprintf('%s (%s) - %s', $_SERVER['HTTP_HOST'], json_encode($_GET), $message));
}
